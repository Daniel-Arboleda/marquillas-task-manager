...
from app.modules.tasks.schemas.task_schemas import TaskCreate, TaskHistoryListResponse, TaskListResponse, TaskResponse, TaskStatus, TaskSummaryResponse, TaskUpdate
...
class TaskService:
    ...
    def create(self, payload: TaskCreate, current_user_id: int, is_admin: bool) -> TaskResponse:
        task = self.command.create(payload, current_user_id, is_admin)
        return self.query.get(task.id)
    ...
    def get(
        ...
        task = self.query.get_entity(task_id)
    ...
    def get_summary(
        ...
        return self.query.get_task_summary(
            current_user_id=current_user_id,
            is_admin=is_admin,
            assigned_user_id=assigned_user_id,
            search=search,
    ...
    def update_status(
        ...
        task = self.command.update_status(
            task=self.query.get_entity(task_id),
            status=status,
            current_user_id=current_user_id,
            is_admin=is_admin,
